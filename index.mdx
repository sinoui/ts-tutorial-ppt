...
```shell 1,2,3,4 title="方式一：先创建ts-hello-world目录，进入这个目录，执行以下命令行"
cd ts-hello-world
yarn init
...
```shell 1 title="如果想在 React 项目中启用 TypeScript，使用create-react-app即可"
npx create-react-app react-ts-hello-world --typescript
```

```shell 1 title="也可以通过ts-lib-scripts创建一个用来练习的 TypeScript 项目"
...
isDone = 2; // Error,不能将“number”类型分配给类型“boolean”
```
...
```ts 8 title="枚举" subtitle="输出Green,因为从枚举来看，Red值为1，Green值为2，Blue值为3"
enum Color {
  Red = 1,
  Green,
  Blue,
